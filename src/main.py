...
if __name__ == '__main__':
    ...
    path = './v2/output/img_new1_undistorted_img_new3_undistorted_matches.npz'
    npz_c1_c3 = np.load(path)
    # path = './output/img_new1_undistorted_img_new4_undistorted_matches.npz'
    # npz_c1_c4 = np.load(path)
    path = './v2/output/img_new1_undistorted_img_old_matches.npz'
    ...
    F, mask = cv2.findFundamentalMat(kp_c1, kp_c3, cv2.RANSAC)
    print(mask.ravel())
    ...
    Kold, R_cold_c1, t_cold_c1 = decomposeP(Pold)
    T_w_cold = ensamble_T(R_cold_c1, t_cold_c1)
    ...
    Pold_optim = refinePoseOldCamera(X3d_optim, kp_old[mask], Pold)

...
def decompose_essential_matrix_v2(x1, x2, E, K, idx):
    # Compute the SVD of the essential matrix
    U, _, V = np.linalg.svd(E)
    t = U[:,2].reshape(-1,1)
    ...
    # print(np.argmax(points_front))
    # T = solutions[np.argmax(points_front)]
    _, v2 = points_in_front_of_both_cameras(x1, x2, solutions[idx], K)
    return solutions[idx], v2
...
if __name__ == '__main__':
    ...
    Rt, X_3d = decompose_essential_matrix_v2(srcPts.T, dstPts.T, E_SG, Kc_new, 0)
    ...
    print(Rt.shape, X_3d.shape)
    drawRefSystem(ax, np.eye(4,4), '-', 'C1')
    drawRefSystem(ax, np.eye(4,4) @ np.linalg.inv(Rt), '-', 'C2_BA_scaled')

    ax.scatter(X_3d[:, 0], X_3d[:, 1], X_3d[:, 2], marker='.', color = 'black') 
    Rt, X_3d = decompose_essential_matrix_v2(srcPts.T, dstPts.T, E_SG, Kc_new, 1)
    print(Rt.shape, X_3d.shape)
    # Rt, X_3d = sfm(Kc_new, F, srcPts.T, dstPts.T, 1)
    if X_3d.shape[0] > 0:
        ax.scatter(X_3d[:, 0], X_3d[:, 1], X_3d[:, 2], marker='.', color='blue')
    Rt, X_3d = decompose_essential_matrix_v2(srcPts.T, dstPts.T, E_SG, Kc_new, 2)
    print(Rt.shape, X_3d.shape)
    # Rt, X_3d = sfm(Kc_new, F, srcPts.T, dstPts.T, 2)
    if X_3d.shape[0] > 0:
        ax.scatter(X_3d[:, 0], X_3d[:, 1], X_3d[:, 2], marker='.', color='green')
    
    Rt, X_3d = decompose_essential_matrix_v2(srcPts.T, dstPts.T, E_SG, Kc_new, 3)
    print(Rt.shape, X_3d.shape)
    # Rt, X_3d = sfm(Kc_new, F, srcPts.T, dstPts.T, 3)
    if X_3d.shape[0] > 0:
        ax.scatter(X_3d[:, 0], X_3d[:, 1], X_3d[:, 2], marker='.', color='red')
    plt.show()
    print(X_3d.shape)
